...
An MCP (Model Context Protocol) server that exposes the OpenAI API as tools for an LLM client to call: `generate_text`, `generate_image`, `create_embeddings`, `list_models`. Published to npm as `@dancommitsgit/openai-mcp`; the compiled `build/index.js` is the package's bin entry point.
